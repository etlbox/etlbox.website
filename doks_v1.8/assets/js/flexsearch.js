...
(function () {
  ...
  function showResults(items) {
    const templateSource = document.getElementById('search-result-template');
    const templateHolder = document.createElement('template');
    templateHolder.innerHTML = templateSource.textContent.trim();
    const template = templateHolder.content;
    ...
    if ((itemsLength === 0) && (query.value === '')) {
      ...
      noResults.append(noResults.dataset.noResultsLabel + ' for "');
      const queryText = document.createElement('strong');
      queryText.textContent = query.value;
      noResults.append(queryText);
      noResults.append('"');
